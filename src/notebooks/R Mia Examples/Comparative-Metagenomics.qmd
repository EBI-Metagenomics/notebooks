...
```{r}
# Define method list
vegdist_methods <- c("manhattan", "euclidean", "canberra", "bray", "kulczynski", "jaccard",
...
Separation of the groups observed in some of the ordination plots could be due to location effect, dispersion effect, or both. To test the location effect (null hypothesis of no difference in composition between groups) we use a permanova implemented in the `vegan` library’s `adonis2` function. We will use the distance metric that best segregates the groups and determine whether the two groups of samples have different centroids. 

1. First we create the metadata data frame.

```{r}
# Create the metadata data frame
metadata <- data.frame(colData(filtered_tse))
rownames(metadata) <- colnames(filtered_tse)
```

2. Then we generate the `mountford` distance matrix.

```{r}
# Transpose the css matrix to test effects in samples
css_data <- t(assay(filtered_tse, "css"))

# Use the vegan package to generate a mountford distance matrix
distance_matrix <- vegan::vegdist(css_data, method = "mountford")
```

3. Run the `adonis2` method.

```{r}
# Perform PERMANOVA using adonis2 from the vegan package - testing location effects
adonis_result <- adonis2(distance_matrix ~ sample_geographic.location..country.and.or.sea.region., data = metadata, perm = 1000)
adonis_result
```
...
```{r}
bd = betadisper(distance_matrix, metadata$'sample_geographic.location..country.and.or.sea.region.')
anova(bd)
```
ANOVA's p-value is signficant (p>0.05), meaning that group dispersins aren't 
homogenous (reject the null hypothesis of no difference in dispersion between groups). The general conclusion of `adonis` and `betadisper` is that our georgaphic groups of samples aren't homogenous on group dispersions (compositions vary significantly within the group) whilst they also have significantly different compositions between groups (according to `adonis` p<0.05).
...
```{r}
# Determine the number of taxa corresponding to the top 25%
total_taxa <- nrow(assay(genus_tse, "counts"))
top_25_percent <- ceiling(total_taxa * 0.25)

# Take the top taxa
top_taxa <- getTopFeatures(genus_tse, method = "prevalence", assay.type = "counts", top = top_25_percent)

# Subset the SummarizedExperiment object to include only the top prevalent taxa
tse <- tse[rownames(tse) %in% top_taxa, ]
```

3. Now we transform the count assay to relative abundances and do abundance filtering.

```{r}
# Apply relabundance transformation
genus_tse <- transformAssay(genus_tse,
                assay.type = "counts",
                method = "relabundance")

# Apply abundance filtering
abundance_cutoff <- 1e-03
abundant_taxa <- rownames(genus_tse)[rowMeans(assay(genus_tse, "relabundance")) >= abundance_cutoff]
genus_tse <- genus_tse[abundant_taxa, ]
```
...
conditions <- ifelse(colData(genus_tse)$sample_geographic.location..country.and.or.sea.region. == "GAZ:United States of America", "USA", "Other")

# Add conditions as a new column in the colData of genus_tse
colData(genus_tse)$geo_location <- conditions
```
...
x <- aldex.clr(assay(subset_genus_tse, "counts"), genus_tse$geo_location)
...
```{r}
colData(genus_tse)$library_size <- colSums(assay(genus_tse, "counts"))
```
